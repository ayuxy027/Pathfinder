...
import { Loader2, BookOpen, Code, Clock, Award, Calendar, BarChart2, Lightbulb, ChevronRight, Star, Zap } from 'lucide-react';
...
export default function Progress() {
  ...
  const getChartOptions = () => ({
    ...
    plugins: {
      legend: {
        ...
        labels: { color: 'rgba(64, 64, 64, 0.8)', font: { size: 11 }, boxWidth: 15, padding: 15 }
      },
      tooltip: {
    ...
    scales: {
      x: { grid: { display: false }, ticks: { color: 'rgba(0, 0, 0, 0.7)', font: { size: 10 } } },
      y: { grid: { color: 'rgba(100, 116, 139, 0.2)' }, ticks: { color: 'rgba(0, 0, 0, 0.7)', font: { size: 10 } } }
    },
  ...
  const getProgressChartData = (metric: MetricType) => ({
    ...
    datasets: [{
      label: metric === 'weekly' ? 'Minutes' : 'Hours',
      data: progressChartData[metric],
          ...
          {[
            ...
            >
              <div className="flex flex-col h-full">
                <div className="mb-3 text-2xl" aria-hidden="true">{stat.icon}</div>
          ...
          >
            <div className="p-6 bg-white rounded-xl border shadow-md border-stone-200/70 backdrop-blur-sm">
              <div className="flex justify-between items-center mb-6">
                ...
                <div className="flex space-x-2">
                  {(['skills', 'progress'] as ChartType[]).map((chart) => (
                    <button
                      key={chart}
              ...
              <div className="h-72" role="img" aria-label={activeChart === 'skills' ? 'Skills proficiency chart' : 'Learning progress chart'}>
                <AnimatePresence mode="wait">
                  {activeChart === 'skills' ? (
                    ...
                    <motion.div key="progress" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full">
                      <div className="flex justify-end mb-4 space-x-2">
                        {(['weekly', 'monthly', 'yearly'] as MetricType[]).map((metric) => (
                          <button
                            key={metric}
            ...
            <div className="p-6 bg-white rounded-xl border shadow-md border-stone-200/70 backdrop-blur-sm">
              <h2 className="flex items-center mb-6 text-xl font-bold text-stone-800">
                <Award className="mr-2 text-teal-500" aria-hidden="true" />
                Recent Achievements
              </h2>
              <div className="space-y-4">
                {achievements.map((achievement) => (
                  ...
                  >
                    <div className="flex justify-center items-center mr-4 w-10 h-10 bg-white rounded-full shadow-sm" aria-hidden="true">
                      {achievement.icon}
                    </div>
                    ...
                    <div className="self-center">
                      <ChevronRight className="text-stone-400" aria-hidden="true" />
                    </div>
                  </div>
                ))}
              </div>
          ...
          >
            <div className="p-6 bg-white rounded-xl border shadow-md border-stone-200/70 backdrop-blur-sm">
              <h2 className="flex items-center mb-5 text-xl font-bold text-stone-800">
                <Calendar className="mr-2 text-teal-500" aria-hidden="true" />
                Learning Streaks
              </h2>
              <div className="grid grid-cols-2 gap-4">
                {[
                  ...
                  { label: 'Total Days', value: String(streakData.totalDays) }
                ].map((item) => (
                  <div key={item.label} className="p-3 text-center rounded-lg bg-stone-50">
              ...
              <div className="mt-6">
                ...
                <div className="grid grid-cols-7 gap-1">
                  {[5, 1, 3, 4, 2, 0, 0].map((intensity, i) => {
                    const colors = [
                      'bg-stone-200', 'bg-teal-100', 'bg-teal-200', 'bg-teal-300', 'bg-teal-400', 'bg-teal-500'
                    ];
                    return (
                      <div
                        key={i}
            ...
            <div className="p-6 bg-white rounded-xl border shadow-md border-stone-200/70 backdrop-blur-sm">
              <h2 className="flex items-center mb-5 text-xl font-bold text-stone-800">
                <Lightbulb className="mr-2 text-teal-500" aria-hidden="true" />
                Learning Insights
              </h2>
              <div className="space-y-4">
                {insights.map((insight) => (
                  <div key={insight.id} className="flex p-3 rounded-lg bg-stone-50">
                    <div className="flex justify-center items-center mr-3 w-8 h-8 bg-white rounded-full shadow-sm" aria-hidden="true">
                      {insight.icon}
                    </div>
              ...
              <div className="mt-4">
                <button type="button" className="flex justify-center items-center py-2 w-full text-sm font-medium rounded-lg transition-colors text-stone-600 bg-stone-100 hover:bg-stone-200">
                  View All Insights
            ...
            <div className="p-6 bg-white rounded-xl border shadow-md border-stone-200/70 backdrop-blur-sm">
              ...
              <div className="space-y-3">
                {[
                  ...
                  <div key={course.title} className="flex items-center p-3 rounded-lg transition-all bg-stone-50 hover:bg-teal-50">
                    <div className="flex-1">
                    ...
                    <button type="button" className="p-1.5 text-white bg-teal-500 rounded-full hover:bg-teal-600" aria-label={`Start ${course.title}`}>
